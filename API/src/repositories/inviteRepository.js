...
class inviteRepository extends BaseRepository {
  ...
  async getInvitesByCompanyId(id){
    const {rows} = await this.query(`SELECT public.users.name, public.users.email, public.invites.isadmin, public.invites.creationdate, public.invites.isvalid, public.invites.area
